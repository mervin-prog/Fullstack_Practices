...
app.post("/upload-pinata",async (req,res)=>{
  try{
    ...
    main(pinataResponse.IpfsHash,res);
  }
  catch(err){
    console.log(err);
  }
...
//code to fetch from pinata API.
async function main(hashID, res) {
  try {
    const result = await fetch(
      `https://blue-adverse-mink-533.mypinata.cloud/ipfs/${hashID}`
    );
    const resData = await result.text();
    console.log(resData);
    res.render("pinata.ejs", {"details": JSON.parse(resData)});
  } catch (error) {
    console.log(error);
  }
}
